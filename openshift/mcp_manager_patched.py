...
class McpManager:
    ...
    async def call_tool(self, qualified_name: str, arguments: Dict) -> Dict:
        ...
        try:
            ...
            # Auto-reconnect for ANY server whose subprocess/session may have
            # died. Builtins re-derive spawn params from _BUILTIN_SERVERS;
            # user-added servers reload them from their DB row. (Local patch:
            # upstream gates this recovery to builtins only, so user-added
            # stdio servers fail permanently once their session dies.)
            ...
            if self.is_builtin(server_id):
                ...
                reconnected = await self._reconnect_user(server_id)
            if reconnected:
                session = self._sessions.get(server_id)
                if session:
                    try:
    ...
    async def _reconnect_user(self, server_id: str) -> bool:
        """Tear down and reconnect a user-added MCP server from its DB row.

        Mirrors _reconnect_builtin for servers registered at runtime, whose
        spawn parameters (command/args/env/url) are persisted in the
        mcp_servers table rather than _BUILTIN_SERVERS.
        """
        try:
            from core.database import McpServer, SessionLocal
        except Exception as e:
            logger.error(f"user MCP reconnect unavailable (no DB access): {e}")
            return False
        db = SessionLocal()
        try:
            ...
            db.close()
        if not srv or not getattr(srv, "is_enabled", True):
            return False
        ...
        try:
            ...
            if ok:
                logger.info(f"Reconnected user MCP server: {srv.name}")
            return ok
        except Exception as e:
            logger.error(f"Failed to reconnect user MCP server {srv.name}: {e}")
            return False
